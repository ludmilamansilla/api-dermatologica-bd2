...
let currentStep = 1;
window.currentStep = currentStep;  // Exponer globalmente
let formData = {
    nombrePaciente: '',
    zonaAfectada: '',
    ...
    fileInput.addEventListener('change', function(e) {
        ...
        reader.onload = function(e) {
            if (previewImg && imagePreview) {
                ...
                uploadArea.style.display = 'none'; // Ocultar el área de upload
            }
        };
...
function mostrarPaso(paso) {
    currentStep = paso;
    window.currentStep = paso;  // Actualizar globalmente
...
function validarPaso(paso) {
    ...
    const alert1 = document.getElementById('validationMessage1');
    const alert2 = document.getElementById('validationMessage2');
    if (alert1) alert1.style.setProperty('display', 'none', 'important');
    if (alert2) alert2.style.setProperty('display', 'none', 'important');
    ...
    if (paso === 1) {
        ...
        if (!nombre) {
            if (alert1) {
                document.getElementById('validationText1').textContent = 'Por favor ingrese el nombre del paciente';
                alert1.style.setProperty('display', 'flex', 'important');
            }
            return false;
        }
        ...
        if (!zona) {
            if (alert1) {
                document.getElementById('validationText1').textContent = 'Por favor seleccione la zona afectada';
                alert1.style.setProperty('display', 'flex', 'important');
            }
            return false;
        }
    ...
    if (paso === 2) {
        if (formData.sintomasSeleccionados.length === 0) {
            if (alert2) {
                document.getElementById('validationText2').textContent = 'Por favor seleccione al menos un síntoma';
                alert2.style.setProperty('display', 'flex', 'important');
            }
            return false;
        }
        return true;
